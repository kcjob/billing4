...
class EquipmentUseDAO
{
  ...
  */
  ...
  static function getEquipmentUseDetails($dbConnection, $name)
  {
    $allDetails = [];

    $query = "SELECT CII.invoice, CII.details, CII.service_id, CII.total, CII.total, CI.filename, ci.payer, cu.email, concat_ws(' ', cu.firstname, cu.lastname) AS name
    FROM core_invoice_item AS CII, core_invoice AS CI, core_users AS CU
    WHERE CII.invoice = CI.number and cu.id = ci.payer and concat_ws(' ', cu.firstname, cu.lastname) = '$name'";
